...
pos_misspelled <-
  mclapply(clean_data$Abstracts, function(x) {
    qdap::which_misspelled(x, dictionary = dictionary)
  }, mc.cores = 30)
...
index <-
  clean_data %>%
  select(index) %>%
  as.data.frame() # To pass error in textProcessor
...
doc_per_topic <- tidy(stm_model) %>%
  group_by(topic) %>%
  slice_max(beta, n = 10) %>%
  aggregate(term ~ topic,
    data = ., paste, collapse =
      " "
  ) %>%
  ungroup() %>%
  rename(`Important words per topic` = term) %>%
  left_join(doc_per_topic, ., by = "topic")
